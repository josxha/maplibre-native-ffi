...
part of its normal package model, as Vala does through VAPI declarations. A
bridge library is built in Rust over the shared `-sys` and ABI-adaptation
crates, then adapted to the target runtime—this is needed when the package
boundary is a native extension or native-method entry point (Python, Node.js,
Java JNI). Both paths keep raw ABI details internal.
