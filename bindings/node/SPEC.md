...
src/map.rs        map handle, map option materialization, style-loading/probes, image source values, style ID lists, style metadata/layer/light JSON/properties, camera commands, repaint, debug-option, and utility proof slices
...
8. Add camera, projection, query, style, and offline APIs. _(Initial map camera
   ...
   projection helper, image source values, style JSON/list/metadata/layer/light/
   property helpers, and style probe proof slices complete.)_
