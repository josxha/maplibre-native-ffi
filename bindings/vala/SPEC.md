...
The scaffold implements one proof slice in Rust:
...
- Resource response descriptors, resource transform/provider callbacks, offline
  operation start/status-result/snapshot/list info/discard helpers, and
...
- Basic style-source operations for GeoJSON URL sources, vector/raster/raster
  ...
  handles, JSON snapshots, DEM helper layers, style JSON/property/light/filter
  operations, basic style-layer lifecycle operations, and map coordinate
...
| ------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| Rust adapter crate  | `bindings/vala/crates/maplibre-native-vala`   | Rust shared library, generated entry stubs, callback trampolines, GObject class implementation. |
...
- `MaplibreNative.supported_render_backends()` returning
  `[Flags] RenderBackendFlags`; raw backend masks stay internal and
  scaffold-only
